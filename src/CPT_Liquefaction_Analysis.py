...
def get_fs_profile(site):
    ...
    res = analyze_Ic_profile(site)
    FC = calculate_fines_content(res["Ic"])
    CRR = calculate_crr(site["qc"].iloc[0], FC, res["sigma_v_eff"], M)
    CSR = calculate_csr(depth, res["sigma_v"], res["sigma_v_eff"], PGA, M)
    FS = CRR / CSR
    FS = np.where(res["Ic"] > 2.6, np.nan, FS)
    return depth, FS

depth0, FS0 = get_fs_profile(site_0)
depth5, FS5 = get_fs_profile(site_5)

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 6), sharey=True)

ax1.plot(FS0, depth0, color="steelblue", label="Class 0 (No manifestation)")
...
ax2.plot(FS5, depth5, color="darkorange", label="Class 5 (Severe lateral spreading)")
...
for i, (site, title) in enumerate(zip(sites, titles)):
    ...
    ax = axes[i]
    ax.plot(FS, depth, color="steelblue", linewidth=1.2)
